...
export default function ApiList() {
  ...
  // Only fetch APIs after auth context confirms user
  useEffect(() => {
    if (user) {
      fetchApis();
    }
  ...
  const downloadDocs = async (api) => {
    ...
    try {
      ...
      doc.text(`Generated by ReqNest - ${user?.email || "User"}`, pageWidth / 2, pageHeight - 10, { align: "center" });
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            ...
            <div className="flex items-center bg-gray-800/50 text-gray-300 px-4 py-2 rounded-xl backdrop-blur-sm border border-gray-700">
              ...
              <span className="text-sm font-medium">{user?.email || user?.name || "Logged in"}</span>
            </div>
          </div>
        </div>
